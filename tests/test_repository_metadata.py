...
def test_citation_metadata_describes_this_project():
    ...
    assert citation["title"] == "lastfm_monitor"
    assert citation["message"]
    assert citation["license"] == "GPL-3.0-or-later"
    assert citation["repository-code"] == "https://github.com/misiektoja/lastfm_monitor"
...
def test_support_document_routes_every_request_type():
    ...
    for destination in ("https://github.com/misiektoja/lastfm_monitor/discussions", "https://github.com/misiektoja/lastfm_monitor/security/advisories/new", "https://github.com/misiektoja/lastfm_monitor/issues/new"):
        assert destination in support
    assert "lastfm_monitor --version" in support
    for concept in ("Last.fm API key", "SMTP passwords", "webhook URLs", "--debug"):
...
def test_local_hooks_run_the_pinned_linter():
    ...
    assert covered.match("lastfm_monitor.py")
    assert covered.match("tests/test_repository_metadata.py")
    assert covered.match("tools/lastfm_wrapped.py")
    assert "lastfm_monitor.py tools tests" in lint_command
