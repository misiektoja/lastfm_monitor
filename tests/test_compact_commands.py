...
monitor = importlib.import_module("lastfm_monitor")
...
def test_display_prefix_is_short(monkeypatch, host, executable, method):
    ...
    monkeypatch.setattr(sys, "argv", ["/private/long/tools/lastfm_monitor.py" if method == "manual" else "/private/long/bin/lastfm_monitor"])
    monkeypatch.setattr(monitor, "__file__", "/private/long/tools/lastfm_monitor.py")
    ...
    expected = executable + " lastfm_monitor.py --setup" if method == "manual" else "lastfm_monitor --setup"
...
def test_help_does_not_display_runtime_paths():
    ...
    assert "lastfm_monitor.py --setup" in result.stdout
