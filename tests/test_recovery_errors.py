...
class TestRendering:
    ...
    def test_the_technical_cause_appears_only_in_debug(self):
        error = pylast.WSError(None, "10", "Invalid API key")
        assert "Technical detail: Invalid API key" in monitor.render_recovery_error(error, debug=True)
...
def test_an_outage_that_changes_category_keeps_its_start(monkeypatch):
    ...
    first = monitor.classify_recovery_error(RuntimeError("503 Server Error"))
...
def test_the_error_printer_classifies_what_it_was_given(capsys):
    ...
    returned = monitor.print_recovery_error(requests.ConnectionError("connection refused"), context="runtime")
...
def test_the_tracker_suppresses_only_the_repeated_fix(capsys):
    ...
    monitor.print_recovery_error(error, "runtime", tracker=tracker)
    monitor.print_recovery_error(error, "runtime", tracker=tracker)
