...
def test_status_reader_preserves_invalid_timestamps(tmp_path, timestamp):
    ...
    path.write_text(json.dumps([timestamp, "artist", "track", "album"]))
...
def test_status_reader_keeps_legacy_trailing_metadata(tmp_path):
    path = tmp_path / "status.json"
    record = [1600000000, "artist", "track", "album", {"note": "kept"}]
...
def test_an_unrenderable_discord_template_is_reported_not_raised(delivery, monkeypatch, capsys, template):
    ...
    assert "WEBHOOK_TEMPLATE cannot render" in error or error == "WEBHOOK_TEMPLATE must be a dictionary or a JSON object string"
    assert monitor.send_webhook("Activity", "Something happened", force=True) == 1
...
def test_a_short_retained_secret_never_replaces_ordinary_words(monkeypatch):
    monkeypatch.setattr(monitor, "LASTFM_API_KEY", "cat", raising=False)
    monkeypatch.setattr(monitor, "WEBHOOK_URL", "https://discord.com/api/webhooks/123456789012345678/private")
...
def test_status_reader_accepts_a_timestamp_the_clock_cannot_support(tmp_path):
    path = tmp_path / "status.json"
    record = [253402214400, "artist", "track", "album"]
    path.write_text(json.dumps(record))
    assert monitor.read_status_record(str(path)) == record
...
def test_reconcile_keeps_the_saved_entry_and_restarts_its_timing(capsys):
    record = [253402214400, "artist", "track", "album", {"owner_note": "keep"}]
...
def test_reconcile_leaves_a_current_record_alone(capsys):
    record = [1600000000, "artist", "track", "album"]
