...
The offline test suite covers everything in `lastfm_monitor.py` that can run without network access. Last.fm and Spotify calls are replaced with test doubles.
...
The test layout mirrors the surfaces a user touches rather than the module layout. See [tests/README.md](https://github.com/misiektoja/lastfm_monitor/blob/main/tests/README.md) for the file-by-file map.
...
* Never use a real Last.fm API key or shared secret, Spotify app credentials, SMTP password or webhook URL.

A change to the monitoring loop, the Spotify metadata backends or Last.fm data handling is not verified by this suite alone. Exercise it against a real account and say so in the pull request, without usernames or credentials.
