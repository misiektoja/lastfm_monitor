...
* Libraries: [pyLast](https://github.com/pylast/pylast), `requests`, `python-dateutil`, `pyotp`, [Spotipy](https://github.com/spotipy-dev/spotipy), `python-dotenv`, `beautifulsoup4`, `curl_cffi`
...
<a id="check-whether-lastfm-monitor-is-already-installed"></a>
...
If both commands work and Python reports version 3.9 or newer, skip to [Install Last.fm Monitor](#install-lastfm-monitor).
...
For display-width-aware terminal truncation, install the optional `wcwidth` package:

```sh
pip install wcwidth
```

Without it the tool runs normally and truncation counts characters instead of display width.
...
```sh
curl -fsSLO https://raw.githubusercontent.com/misiektoja/lastfm_monitor/refs/heads/main/lastfm_monitor.py
curl -fsSLO https://raw.githubusercontent.com/misiektoja/lastfm_monitor/refs/heads/main/requirements.txt
```

You can also download [lastfm_monitor.py](https://raw.githubusercontent.com/misiektoja/lastfm_monitor/refs/heads/main/lastfm_monitor.py) and [requirements.txt](https://raw.githubusercontent.com/misiektoja/lastfm_monitor/refs/heads/main/requirements.txt) in a browser or use the files from a cloned repository.
...
For optional display-width-aware truncation install `wcwidth`:
...
```sh
python3 lastfm_monitor.py --version
```

Use `python lastfm_monitor.py --version` on Windows.
...
<a id="upgrade-a-pypi-installation"></a>
### Upgrade a PyPI Installation

```sh
pip install --upgrade lastfm_monitor
lastfm_monitor --version
```
...
<a id="upgrade-a-manual-installation"></a>
### Upgrade a Manual Installation

Replace [lastfm_monitor.py](https://raw.githubusercontent.com/misiektoja/lastfm_monitor/refs/heads/main/lastfm_monitor.py) and [requirements.txt](https://raw.githubusercontent.com/misiektoja/lastfm_monitor/refs/heads/main/requirements.txt) with the newest copies. You can download them in a browser, use the files from an updated clone or run:
...
Use `python lastfm_monitor.py --version` on Windows. If you modified the script itself, save your changes before replacing it and reapply them to the new version.

<a id="check-upgrade"></a>
### Check Upgrade
