...
Version **2.7** adds a **`--setup` wizard** that writes a ready-to-run configuration, a **`--doctor` preflight** that checks a setup before monitoring starts, a **welcome screen** that shows a first-time user which command to run, a **hidden prompt for the SMTP password** that verifies it against the mail server, **coloured terminal output** and **Last.fm profile change tracking** for the public display name and About Me bio. A new **`--verbose` mode** reports the decisions a run made and **`--debug` now traces every operation**, including the failures the tool used to swallow. The documentation now lives on its **own searchable website** instead of one long README. It also clarifies **ntfy webhook customization**. Release downloads are now verifiable, the repository can be cited directly from its GitHub page and an automated defect check runs on every change. **Configuration files are now read as data instead of executed**, so a configuration file sitting in the working directory can no longer run code. **Generating a configuration over an existing file now asks first and keeps a timestamped backup.** **Secrets exported as environment variables now work without a dotenv file** and take precedence over one. **Follower and following alert emails list each changed user on its own line** again. The project itself gains a published security policy with private vulnerability reporting, guided issue and pull request templates, contribution and dependency licensing documentation plus a PyPI release that cannot publish until the full test suite passes.

**Features and improvements**:

- **NEW:** **`--doctor` preflight** - Run **`lastfm_monitor --doctor <lastfm_username>`** to check a setup before starting monitoring. It reports the Python version and the libraries in use, the configuration and dotenv files in effect, which source supplied each secret, every file the run will write, the connectivity endpoint, the Last.fm credentials, the Spotify metadata backend, the monitored profile and both notification channels. Each row is one result marked **`[PASS]`**, **`[WARN]`**, **`[FAIL]`** or **`[SKIP]`**, and a row that needs action carries the same `To fix:` line and guide link the tool prints elsewhere. **Every optional library row says what it is used for**, so a missing one can be judged against the feature it affects. Credentials are checked with a real API call rather than by looking for a non-empty value, and are never displayed. **No files are written**, and the optional email and webhook delivery tests each send one real message only after you approve them. The exit code is `1` when anything failed and `0` otherwise, so it can gate a deployment
- **NEW:** **Guided setup wizard** - Run **`lastfm_monitor --setup`** to answer a few questions and get a ready-to-run configuration. It covers the monitored user, the polling intervals, the Last.fm API credentials, optional Spotify track details, follower and profile tracking, the files a run writes, email notifications and webhook alerts. **Nothing is written until you choose Save** on the summary, which can be reviewed section by section so one answer changes without repeating the rest. **Secrets go to the dotenv file and never to the config file**, an existing config is replaced only after you agree and a timestamped backup is kept. Credentials are checked while you enter them: the Last.fm key and shared secret against Last.fm, the mail server through a real sign-in that sends nothing. **An ntfy webhook takes the topic name on its own** when the topic lives on `ntfy.sh`, so typing `my-topic` saves `https://ntfy.sh/my-topic`. Alerts that this setup cannot produce, such as follower alerts without follower tracking, are not offered. **No question can trap you**: an answer the wizard cannot use is explained and offered again, and declining keeps the default the prompt showed, switches that channel and its alerts off or ends the section. After saving it offers `--doctor` and then the command that starts monitoring. The wizard needs an interactive terminal and says so when there is none
- **NEW:** **The config file can name the user to monitor** - The new **`LASTFM_USERNAME`** setting saves the monitored user, so `lastfm_monitor` on its own starts monitoring instead of printing the welcome screen. A username passed on the command line overrides the saved one for that run. `--setup` writes it after asking whether to persist the target
- **NEW:** **Profile change tracking** - Track the editable **About You** text shown publicly as **About Me** with `TRACK_BIO` or `--track-bio`. Track the public **display name** with `TRACK_DISPLAY_NAME` or `--track-display-name`. Both fields use the existing `FRIENDS_CHECK_INTERVAL` timer plus the same consecutive confirmation and retry controls as follower or following changes. Confirmed values persist in `lastfm_<username>_profile.json` across restarts. Console output is automatic while `PROFILE_NOTIFICATION` / `--notify-profile` and `WEBHOOK_PROFILE_NOTIFICATION` / `--webhook-profile` control email or webhook delivery. The startup summary gives each tracked field its own row, next to the shared check interval
- **IMPROVE:** **A startup summary you can read at a glance** - The settings block printed before monitoring starts now shows only what matters for this run: the monitored user, the polling intervals, both alert channels, the files in use and each optional feature that is switched on. Everything else moves to **`--verbose` or `--debug`**, which print the complete list including the settings left at their defaults, the install method, where each secret came from and the state file the run keeps. **The log file keeps the complete list either way**, so a log attached to a bug report carries every effective setting. Values line up in one column instead of drifting with the terminal's tab stops, and the labels are shorter and consistent: **`Config`**, **`Dotenv`**, **`CSV output`**, **`Output logging`**, **`Liveness output`** and **`Monitored-track alerts`**
- **NEW:** **`--verbose` mode and a `--debug` trace that names every operation** - The new **`--verbose`** flag and **`VERBOSE_MODE`** setting report the decisions a run made: a channel switched off because its settings cannot work, a tracked field that could not be read together with the alert that silences and the point where that feature works again. **A lasting outage is reported once rather than on every check**, and a recovery is reported only when the failure itself was printed. **`--debug`** now covers every outbound call with its address, timeout, status and result, the configuration load, every completed check and the wait before the next one, every retry with its delay, each file the run reads or writes and both notification channels including whether the message was delivered. **Failures the tool recovers from silently are named too**, so a feature that quietly does nothing can be diagnosed. Every debug line now reads `Operation: key=value, key=value` and reports `outcome=OK`, `failed`, `degraded` or `skipped`, so `grep outcome=failed` finds every failure in a long run. The two modes are **independent**, each flag wins over a configuration file that switches its mode off, and a run with neither ends its startup summary with a line naming both
...
- **NEW:** **Screen width cap** - The new **`TRUNCATE_CHARS`** setting and **`--truncate N`** flag cut each printed line to a maximum width, so long track titles and file paths stop wrapping. **`999`** auto-detects the terminal width and reports what it found. **The log file always keeps the full line**, which is why the setting is ignored when logging is disabled with `-d`. It is off by default and needs the optional `wcwidth` library to measure display width, otherwise lines are left untouched. The startup summary names the width in effect
...
- **IMPROVE:** **A failed alert channel is retried on its own** - Sending an alert used to count as delivered whether or not it arrived, so an error email the mail server rejected was never tried again. **Each channel now records whether it actually arrived**, and while a failure lasts the next check retries only the channel that failed. The channel that already delivered is left alone, so nothing arrives twice
- **IMPROVE:** **Text from Last.fm cannot drive your terminal** - Track, artist and album names, display names and About Me text are chosen by other people, and they used to reach the console and the log file exactly as Last.fm returned them. **Terminal control sequences are now removed from every line the tool prints**, so a scrobble titled with an escape sequence can no longer clear the screen, retitle the window or hide text when the output is read or a log file is opened later. The same removal applies with `DISABLE_LOGGING = True`, which previously had nothing between the tool and the terminal at all. HTML email bodies already escaped these names and still do. Tabs and newlines are untouched, so reports and logs look the same as before. **One exception comes with coloured output**: because the tool's own colours travel the same path, a plain colour change is now kept rather than stripped, so a scrobble title carrying one changes how the rest of that line looks. It cannot move the cursor, clear the screen or set the window title, and log files still have every escape removed
- **NEW:** **Coloured terminal output** - Live output is now coloured by what each value is rather than by which row it sits in, so the monitored account, links, track and album names, dates, durations, counters and listening states each read at a glance. The same colours cover the startup summary, the `--doctor` report and its `[PASS]` / `[WARN]` / `[FAIL]` / `[SKIP]` markers, the setup wizard's headings, prompts and menus, the welcome screen and the recent-tracks table. **`COLORED_OUTPUT`** switches it off, **`--no-color`** does the same for one run and colour also switches itself off when output is redirected or piped, when `TERM` is unset or `dumb` and when the standard `NO_COLOR` environment variable is set. **`COLOR_THEME`** overrides individual colours and ships commented out, so the built-in defaults apply and a later change to them reaches you. **Log files are always written plain**, so a log attached to a bug report carries no escape sequences. Part names are shared with the sibling monitors, so a `COLOR_THEME` block can be copied between them. On Windows, install the optional `colorama` package for the classic Command Prompt
...
- **IMPROVE:** **The liveness reminder says what it is** - While nothing changes the tool used to print a bare `Liveness check, timestamp: ...` line, twice a day by default, with nothing explaining it. It now prints **`Monitoring healthy for <username>`** with the user's current state on the line above that timestamp. The reminder also **arrives while the user is listening**, where it previously never appeared at all, so a run watching someone who plays music for days no longer looks dead. It is now timed in seconds, so it keeps to `LIVENESS_CHECK_INTERVAL` at both polling rates, and anything the tool prints about the user starts the countdown again. Set the interval to 0 to switch the reminder off
- **IMPROVE:** **A failing run reports once instead of on every check** - A failure is now reported the moment it starts, with the delay before the next attempt on the same line: **`* Error: The Last.fm API is temporarily unavailable (retrying in 30 seconds)`**. Previously the first failures printed nothing at all until a threshold was reached, and after that the same block repeated on every check, which at a 30 second interval is thousands of identical blocks in a two day outage. While a failure lasts the run now stays quiet and the liveness reminder carries it as **`Monitoring degraded for <lastfm_username>`** with the summary and the time it started. When it clears the run prints **`Monitoring recovered for <lastfm_username> after <duration>`**, whatever flags it was started with. Failures are told apart by category rather than by matching the error text, so a rate limit that follows an outage is reported in full instead of being hidden by it, and **error emails and webhook alerts go out once a failure has lasted two minutes** rather than after the counted threshold, while a rejected API key is alerted at once. A Last.fm blip of a few checks therefore reaches nobody, which matters at a polling interval of a few seconds. With **`LIVENESS_CHECK_INTERVAL = 0`** there is no reminder to carry an outage, so the aggregated `Error 50x (15x times in the last 2 minutes)` and network summaries keep their `ERROR_500_NUMBER_LIMIT` and `ERROR_NETWORK_ISSUES_NUMBER_LIMIT` cadence
- **IMPROVE:** **A command with no arguments shows how to start** - Running `lastfm_monitor` with nothing after it used to print the full option list, over 130 lines, to standard error. It now prints the form the username takes and four commands: start monitoring, run the guided setup, check the setup with `--doctor` and list recent tracks. On a terminal it also offers to run the guided setup straight away. Each one is written for the way you installed the tool, and the screen ends with `--help` and a link to the setup guide. The exit code stays `1`, since the command was still incomplete
- **IMPROVE:** **A startup banner shared with the sibling monitors** - The tool now opens with the same ASCII banner the other monitors print, a boxed record beside the Last.fm name with the version under it, replacing the single `Last.fm Monitoring Tool v2.7` line. `--version` still prints one line on its own
- **IMPROVE:** **The screen is cleared only when monitoring starts** - Commands that print a result and exit no longer wipe the terminal first, so a `--doctor` report can be compared against the previous run and `--help`, `--send-test-email`, `--send-test-webhook` and the `--set-*` commands keep what was already on screen. A `--debug` run keeps the scrollback too. `CLEAR_SCREEN` still controls the clear that monitoring starts with
- **NEW:** **`--set-smtp-password`** - Enter the SMTP password through a hidden prompt instead of editing the dotenv file by hand. The command **signs in to the configured mail server and saves the password only if the server accepts it**, so a password the provider rejects, usually because it wants an app password, is caught at setup rather than at the first alert. Nothing is sent. A refused sign-in, a declined replacement and incomplete mail settings each leave the dotenv file untouched, and incomplete settings are reported before anything is typed
- **IMPROVE:** **Test messages say which tool and which command sent them** - `--send-test-email` now arrives as **`lastfm_monitor: test email`** with the body `This test email was sent by --send-test-email. Your SMTP settings work.`, replacing `This is test email - your SMTP settings seems to be correct !`. The test webhook is titled **`lastfm_monitor: test webhook`** instead of `Last.fm Monitor test`, and the delivery tests `--doctor` offers say they came from the doctor. A message can arrive minutes later beside alerts from another monitor, so it names both the tool and the command. **A test command with nothing to send now reports the missing settings before announcing a send**, in the same error, fix and guide form as the rest of the tool
- **IMPROVE:** **`--generate-config FILENAME` never replaces a file silently** - Writing the template over an existing configuration used to overwrite it with no copy and no question. On a terminal the tool now asks before replacing it, and outside one, in a script or a container, it stops and names the new **`--force`** flag. Either way the previous file is copied to **`<name>.<timestamp>.bak`** first and the backup path is printed. Both files are readable only by their owner. Shell redirection with `>` truncates the destination before the tool starts, so pass the filename when the file already exists
- **IMPROVE:** **State files survive an interrupted write** - The last activity file, the follower and following state files and the profile state file are now replaced in one step instead of being rewritten in place. A crash or a full disk during a save previously left a half-written file, which the next start could not read, so the baseline was lost and the first check reported every follower and every field as changed
- **IMPROVE:** **`--config-file none` runs on the built-in defaults** - `--env-file none` already switched the dotenv search off, while `--config-file none` failed with `Config file 'none' does not exist`. Both flags now take the sentinel, so a run can ignore any configuration file and use the built-in defaults with command-line flags, without picking up a `lastfm_monitor.conf` from the working directory. Both say so in `--help`, and a path that does not exist is still an error
- **IMPROVE:** **A `--help` screen that groups what it lists** - The email flags now sit under **`Email notifications`** rather than a bare `Notifications` next to `Webhook notifications`, and the listing flags under **`User information & listing`**. The screen ends with worked **examples grouped by task** - getting started, notifications, listening extras, information and diagnostics - each with a comment saying what it is for, written for the way you installed the tool, followed by a link to the setup guide
...
- **IMPROVE:** **The GRC recipe matches the live colours** - The bundled `grc/conf.monitor_logs` now paints a saved log the way the terminal paints it while the tool runs. One copy in `~/.grc/` covers every monitor in the family. Copy the updated file over your existing one
...
- **BUGFIX:** **`LIVENESS_CHECK_INTERVAL` is read from the configuration file** - The reminder interval was taken from the shipped 12 hour default unless `--check-interval` was also passed, so setting it in a configuration file changed the startup summary but not how often the reminder actually arrived. The configured value is now used
- **BUGFIX:** **A refused argument combination names what you typed** - A `--set-*` command combined with another argument reported that argument by its internal name, so `--set-webhook-url --debug` answered `cannot be combined with --debug-mode`, naming a flag that does not exist, and adding a username answered `--username` for a value that is positional. The message now names the option as it was typed, or `LASTFM_USERNAME` for the username
- **BUGFIX:** **Backing out of a `--set-*` command says which answer you gave** - Pressing **Ctrl+C** at a prompt printed `You pressed Ctrl+C, tool is terminated.` and **exited with a success code**, so a script could not tell a cancelled setup from a completed one. It now reports the cancellation and exits with a failure code. Answering **`n`** to the replacement question was also reported as a cancellation, with advice to run the command again once you have the value ready, which only asks the same question. It now says the saved value was left as it is and names the answer that replaces it. Neither answer changes the dotenv file
- **BUGFIX:** **Redirected output no longer starts with a terminal warning** - Piping a run to a file put `TERM environment variable not set.` on the first line of the transcript, printed by the `clear` command, which then arrived in bug reports. The screen is no longer cleared when output is redirected
- **BUGFIX:** **`CLEAR_SCREEN` is read from the configuration file** - The setting was applied after the screen had already been cleared, so `CLEAR_SCREEN = False` in a configuration file had no effect. It is now read before the clear
...
- **BUGFIX:** **The webhook provider warning names the service** - When a configured `WEBHOOK_PROVIDER` did not match the `WEBHOOK_URL`, the warning that reported the correction printed the stored value, so a Discord URL read `Using discord`. It now names the service the way the service spells it and ends the sentence
