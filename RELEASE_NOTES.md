...
- **IMPROVE:** **A command with no arguments shows how to start** - Running `lastfm_monitor` with nothing after it used to print the full option list, over 130 lines, to standard error. It now prints the form the username takes and four commands: start monitoring, save the Last.fm API credentials, check the setup with `--doctor` and list recent tracks. Each one is written for the way you installed the tool, and the screen ends with `--help` and a link to the setup guide. The exit code stays `1`, since the command was still incomplete
