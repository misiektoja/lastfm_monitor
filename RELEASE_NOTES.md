...
- **IMPROVE:** **A startup summary you can read at a glance** - The settings block printed before monitoring starts now shows only what matters for this run: the monitored user, the polling intervals, both alert channels, the files in use and each optional feature that is switched on. Everything else moves to **`--verbose` or `--debug`**, which print the complete list including the settings left at their defaults, the install method, where each secret came from and the state file the run keeps. Values line up in one column instead of drifting with the terminal's tab stops, and the labels are shorter and consistent: **`Config`**, **`Dotenv`**, **`CSV output`**, **`Output logging`**, **`Liveness output`** and **`Monitored-track alerts`**
